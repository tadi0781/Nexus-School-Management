...
def contacts_list():
    ...
    if "teacher" in allowed_role_names:
        ...
        all_teachers = db.session.scalars(
            select(User)
            .where(User.role.has(name="teacher"), User.is_active == True)
...
def view_notifications():
    ...
    if unread_ids_to_mark:
        try:
            ...
            db.session.commit()
            app.logger.info(
                f"Marked {len(unread_ids_to_mark)} notifications as read for user {current_user.username} upon viewing /notifications page."
            )
            ...
            db.session.rollback()
            app.logger.error(
                f"Error marking notifications as read for user {current_user.id} on /notifications view: {e}",
                exc_info=True,
            )
...
def notification_contacts_for_sending():
    ...
    allowed_receiver_role_names = get_allowed_notification_roles(
...
#      pass
# In app.py, replace the existing send_notification_to_user route with this.
...
def send_notification_to_user(receiver_id):
    ...
    if not is_notification_allowed(current_user, receiver):
        flash(
            f"You do not have permission to send notifications to {receiver.full_name or receiver.username}.",
            "danger",
        )
    ...
    if form.validate_on_submit():
        try:
            notification = Notification(
                ...
                timestamp=datetime.now(timezone.utc),
                notification_type="manual_general",
            )
            db.session.add(notification)
            db.session.commit()

            # --- FIX: REAL-TIME NOTIFICATION PUSH ---
            # After committing, the notification has an ID and all its data.
            # Now, emit an event to the receiver's personal room.
            try:
                # Eagerly load sender for rendering the partial template
                notification_to_render = (
                    db.session.query(Notification)
                    .options(joinedload(Notification.sender))
                    .get(notification.id)
                )

                # Render the HTML snippet for the new notification
                notification_html = render_template(
                    "partials/_notification_item.html",
                    notification=notification_to_render,
                )

                # Get the new total unread count for the receiver
                new_unread_count = get_unread_notifications_count(receiver.id)

                # Emit the event to the receiver's specific room
                socketio.emit(
                    "new_notification",
                    {"html": notification_html, "unread_count": new_unread_count},
                    room=f"user_room_{receiver.id}",
                )

                app.logger.info(
                    f"Emitted 'new_notification' event to room for user {receiver.id}"
                )

            except Exception as socket_e:
                # Log error but don't fail the entire request if socket emit fails
                app.logger.error(
                    f"Failed to emit SocketIO notification for user {receiver.id}: {socket_e}",
                    exc_info=True,
                )
            # --- END OF REAL-TIME FIX ---

            flash(
                f"Notification sent successfully to {receiver.full_name or receiver.username}!",
                "success",
            )
            ...
            db.session.rollback()
            app.logger.error(
                f"Error sending notification from {current_user.id} to {receiver.id}: {e}",
                exc_info=True,
            )
            flash("Failed to send notification due to a server error.", "danger")

    return render_template(
        "notifications/send_form.html",
        form=form,
        receiver=receiver,
        title=f"Send Notification to {receiver.full_name or receiver.username}",
    )
...
def social_feed_placeholder():
    ...
    return render_template(
        "social/global_feed_placeholder.html",
        title="Global Feed",
